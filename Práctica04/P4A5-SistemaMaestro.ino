...
void setup() { 
Wire.begin(); //Iniciamos la comunicación con los nodos esclavos 
Serial.begin(9600); // Comunicacion a 9600 baudios
lcd.init(); // Se incia el lcd
...
if(roll == -90){ // Implementamos las condiciones para cumplir con la tabla proporcionada de requerimientos en la practica 
angulo = 0;
}
else {
if(roll == 0) {
...
if(roll >= -89 and roll <=-1) { // Mapeamos los valores recibidos para cumplir con la tabla proporcionada, dado que los 
  // valores implican un intervalo, es necesario usar la funcion map
...
lcd.setCursor(4,0); // Se posiciona en la cuarta columna de la fila inicial
lcd.print("Angulo: "); // Imprime textualmente en la pantalla lcd "Angulo: "
lcd.print(roll); // Imprime el contenido de roll
lcd.setCursor(4,1); // Pocisionamos el cursor del lcd en la cuarta columna de la fila de abajo
lcd.print("DatoIC2: "); // Muestra DatoIC2: en pantalla lcd
lcd.print(angulo); // imprime el valor del angulo que moveremos en el servo en el ldc
